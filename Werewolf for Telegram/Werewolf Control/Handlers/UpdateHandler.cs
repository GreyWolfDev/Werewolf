...
//#endif
        ...
        private static string[] nonCommandsList = new[] { "vote", "getlang", "validate", "setlang", "groups", "status", "done", "gif", "joinbutton", "fleebutton", "stopwaiting" };
            ...
            using (var DB = new WWContext())
            {
                try
                {
                    ...
                    if (args[0] == "joinBtn" || args[0] == "fleeBtn")
                    {
                        ...
                        int n, guid;
                        if (!(int.TryParse(nodeid, out n) && int.TryParse(gameid, out guid)))
                            return;

                        //first get the node where to search for the game
                        Models.Node nodeGame = null;
                        for (var i = 0; i < 3; i++)
                        {
                            nodeGame = Bot.Nodes.ToList().FirstOrDefault(x => x.ClientId == n);
                            if (nodeGame != null) break;
                        }
                        if (nodeGame == null)
                        {
                            //log it
                        ...
                        for (var i = 0; i < 5; i++)
                        {
                            game = nodeGame.Games.ToList().FirstOrDefault(x => x.Guid == guid);
                            if (game != null) break;
                        }
                        if (game == null)
                        {
                        ...
                        {
                            Bot.ReplyToCallback(query, "Leaving...", false);
        ...
        internal static InlineKeyboardMarkup GetConfigMenu(long id)
        {
            ...
            buttons.Add(new InlineKeyboardButton("Allow Fleeing", $"flee|{id}")); //TODO add
            ...
            //buttons.Add(new InlineKeyboardButton("Allow Tanner", $"tanner|{id}"));  //DONE
            //buttons.Add(new InlineKeyboardButton("Allow Cult", $"cult|{id}"));
            //buttons.Add(new InlineKeyboardButton("Enable Secret Lynch", $"secretlynch|{id}"));  //DONE
