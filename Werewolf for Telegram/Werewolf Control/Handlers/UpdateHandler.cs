            ...
            using (var DB = new WWContext())
            {
                try
                {
                    if (String.IsNullOrEmpty(query.Data))
                    {
                        ...
                        ReplyToCallback(query, "Invalid Callback");
                        return;
                    }
                    ...
                    if (choice == "cancel")
                    {
                        ReplyToCallback(query, GetLocaleString("WhatToDo", language), replyMarkup: GetConfigMenu(groupid));
                        return;
                    }
                    if (!nonCommandsList.Contains(command.ToLower()))
                        ...
                        {
                            ReplyToCallback(query, GetLocaleString("GroupAdminOnly", language), false);
                            return;
                        }
                    ...
                    switch (command)
                    {
                        case "status":
                            if (args[3] == "null")
                            {
                                ...
                                ReplyToCallback(query, "Set status to?", replyMarkup: menu);
                            }
                            else
                            {
                                //update the status
                                ...
                                ReplyToCallback(query, "Status updated");
                            }
                            break;
                        case "groups":
                            ...
                            ReplyToCallback(query, GetLocaleString("HereIsList", language, choice));
                            if (groups.Count() > 5)
                            {
                                //need to split it
                        ...
                        case "getlang":
                            if (choice == "All")
                            {
                                ReplyToCallback(query, "One moment...");
                            ...
                            else
                            {
                                ...
                                if (isBaseG)
                                {
                                    ...
                                    if (variants.Count() > 1)
                                    {
                                        ...
                                        ReplyToCallback(query, GetLocaleString("WhatVariant", language, " "),
                                            replyMarkup: menu);
                                        return;
                                    }
                        ...
                        case "upload":
                            ...
                            if (choice == "current")
                            {
                                ReplyToCallback(query, "No action taken.");
                                return;
                            }
                        ...
                        case "lang":
                            ...
                            ReplyToCallback(query, GetLocaleString("WhatLang", language, curLang.Base), replyMarkup: menu);
                            break;
                        case "setlang":
                            //first, is this the base or variant?
                            ...
                            if (isBase)
                            {
                                ...
                                if (variants.Count() > 1)
                                {
                                    ...
                                    ReplyToCallback(query, GetLocaleString("WhatVariant", language, curVariant.Variant),
                                        replyMarkup: menu);
                                    return;
                                }
                                //only one variant, move along
                                                ...
                                                StringComparison.InvariantCultureIgnoreCase)))
                            {
                                ...
                                if (grp != null)
                                {
                                    ...
                                    ReplyToCallback(query, GetLocaleString("WhatToDo", language), replyMarkup: menu);
                                }
                                if (p != null)
                                {
                                    p.Language = lang.FileName;
                                    ReplyToCallback(query, GetLocaleString("LangSet", language, lang.Base + (String.IsNullOrWhiteSpace(lang.Variant) ? "" : ": " + lang.Variant)));
                                }
                            }
                            DB.SaveChanges();
                            break;
